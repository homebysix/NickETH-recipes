...
# Adapted for Windows, 20200618, Nick Heim, ETH Zuerich
#
...
# Adapted to Vmware Remote Console for Windows. Hm, 20200618
...
FUSION = "vmrc-windows.xml"
DEFAULT_MAJOR_VERSION = "7"


class VMwareVMRCURLProvider(URLGetter):
    """Processor class."""
    ...
    def core_metadata(self, base_url, product_name, major_version):
        ...
        build_re = re.compile(r"^vmrc\/([\d\.]+)\/(\d+)\/")

        versions = {}
        for metadata in metaList:
            version = metadata.find("version")
            print(r"metadata-version %s" % version.text, file=sys.stdout)
